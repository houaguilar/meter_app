// lib/presentation/providers/providers.dart

// Providers de Ladrillo
export 'ladrillo/ladrillo_providers.dart';

// Providers de Pisos
export 'pisos/falso_piso_providers.dart';
export 'pisos/contrapiso_providers.dart';
export 'pisos/pisos_providers.dart';

// Providers de Tarrajeo
export 'tarrajeo/tarrajeo_providers.dart';

// Providers de Losas
export 'losas/losas_aligeradas_providers.dart';

// Providers de Estructuras
export 'home/estructuras/structural_element_providers.dart';

